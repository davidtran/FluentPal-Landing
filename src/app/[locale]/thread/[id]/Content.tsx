...
export const Content = () => {
  ...
  async function fetchReplay() {
    ...
    setIsLoading(true);
    try {
      const res = await axios.get(url + "/threads/details", {
        params: { thread_id: id },
      });
  ...
  function startReplay() {
    ...
    audio.current = new Audio(replay.messages[0].cdn_url);
    audio.current.play();
  ...
  function playChatBubble(index: number) {
    ...
    if (message.cdn_url) {
      setTimeout(() => {
        ...
        audio.current = new Audio(message.cdn_url);
        audio.current.play();
        audio.current.addEventListener("ended", onAudioEnd);
        setIsPlaying(true);
      }, 300);
  ...
  return (
    <>
      <div
        ref={containerRef}
        id="container"
        className="relative mx-auto max-w-[90%] w-[600px] flex-1 bg-white p-[15px] rounded-[20px] shadow-sm overflow-scroll"
      >
        {isLoading && (
          <Center>
            <Loader />
        ...
        {replay && (
          <div className={classNames("relative")}>
            ...
            >
              {replay.messages.map((item: any, index: number) => {
        ...
        {!isReplayEnded && isStarted && (
          <div
            ...
            className="fixed bg-[rgba(255,255,255,0.4)] w-[50px] h-[50px] border-[#C3C3C3] border-[1px] shadow-md rounded-full flex justify-center items-center bottom-[55px] z-[9999] cursor-pointer left-1/2 -translate-x-1/2"
          >
            {!isPlaying ? (
              <img
                src="/play.svg"
